...
//             </p>
//             <button className="mt-4 bg-[#2a2a3c] border-[#3a3a4c] hover:bg-[#3a3a4c] px-4 py-2 rounded">
//               Add Tickers
//             </button>
//           </div>
//         )}
...
const StockWatchlist = ({ sendMessageToChat }) => {
  ...
  // Replace Finnhub news function with Alpha Vantage
  const getCompanyNews = async (symbol) => {
    try {
      const response = await axios.get(
        `https://www.alphavantage.co/query?function=NEWS_SENTIMENT&tickers=${symbol}&apikey=04RGF1U9PAJ49VYI`
      );
      ...
      // Take top 3 news items and format them
      return response.data.feed.slice(0, 3).map((item, index) => ({
        id: `${symbol}_${index}`,
        headline: item.title,
        summary: item.summary,
        source: item.source,
        ...
        datetime: new Date(
          item.time_published.substring(0, 4) +
            ...
            "-" +
            item.time_published.substring(6, 8) +
            "T" +
            item.time_published.substring(9, 11) +
            ":" +
            item.time_published.substring(11, 13)
        ),
        category: item.topics?.[0]?.topic || "Finance",
      }));
    } catch (error) {
